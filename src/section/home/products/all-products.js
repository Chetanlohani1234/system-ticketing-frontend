...
const AllProducts = () => {
    ...
    const getData = () => {
        DataService.getProduct().then((data) => {
            console.log(data.data.data)
            setData(data?.data?.data);
            setfilteredData(data?.data?.data)
            setTotalPages(Math.ceil(data?.data?.products_count / 15))
            setLoading(false);
        }).catch((error) => {
            setLoading(false);
    ...
    const onChangeSearch = (e) => {
        ...
        if (e.target.value) {
            const result = data.filter(value => {
                return value.productName.toLowerCase().includes(e.target.value.toLowerCase()) ||
                    value.SKU_Number.toLowerCase().includes(e.target.value.toLowerCase()) ||
    ...
    const deleteProduct = (item) => {
        ...
        setLoading(true);
        DataService.DeleteProduct(data, item?._id).then(
            () => {
    ...
    const renderPageButtons = () => {
        ...
        for (let page = startPage; page <= endPage; page++) {
          pageButtons.push(
            <li  className={(currentPage === page) ? "active" : ""} > <a style={{cursor:'pointer'}}
              key={page}
    ...
    return (
        <div className="row">
            ...
            <div className="col-md-12">
                <h4 className="f-700 mb-4">All Products</h4>
                {message && (
                ...
                <div className="table-header d-flex align-items-center">
                    <div className="table-search">
                        ...
                        <input type="search" onChange={onChangeSearch} name="search" placeholder="Search product" />
                    </div>
                            ...
                            ? filteredData.map((item, i) => (
                                <tr>
                                    ...
                                    <td className="d-flex align-items-center">
                                        {(item.images[0] ?
                                            <span className="thumb-img me-2"><img src={"https://https://opm-stream.onrender.com/" + item.images[0].path} className="product-img" alt="product" /></span>
                                            : ""
                                        )}{item.productName}
                                    </td>
                                    ...
                                    <td>{item.MRP}</td>
                                    <td width="150"><span >{item.status}</span></td>
                                    <td width="150"><span className="d-flex justify-content-end"><Link to={"/view-product/" + item._id} className="mx-2"><svg width="17" height="12" viewBox="0 0 17 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <Link to={"/edit-product/" + item._id} className="mx-2"><svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <ul className="pagination mt-5">
                    ...
                    {currentPage > 1 && (
                        <li><a style={{cursor:'pointer'}} onClick={() => goToPage(currentPage - 1)}>&#8592;</a></li>
                    )}
