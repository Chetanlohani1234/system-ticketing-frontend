...
const AllTicket = () => {
    ...
    // const getData = () => {
    //     if(storedUserId){
    //         DataService.getVenue(storedUserId).then((data) => {
    ...
    const getData = () => {
            DataService.getTicket().then((data) => {
               // console.log("saddsd",data?.data?.data);
                setData(data?.data?.data);
                setfilteredData(data?.data?.data)
                setLoading(false);
            }).catch((error) => {
    ...
    const deleteTicket  = (item) =>{
        setLoading(true);
        DataService.deleteTicket(item?._id).then(
            () => {
    ...
    return (
        <div className="row">
            ...
            <div className="col-md-12">
                <h4 className="f-700 mb-4">All Ticket</h4>
                {message && (
                ...
                <div className="table-header d-flex align-items-center">
                    <div className="table-search">
                        ...
                        <input type="search" onChange={onChangeSearch} name="search" placeholder="Search Venue" /> */}
                    </div>
                    ...
                    <form class="d-flex align-items-center ms-auto">
                        ...
                        <Link to={"/add-ticket"} className="btn btn-secondary">Add Ticket</Link>
                            ...
                            ? data.map((item, i) => (
                                <tr>
                                    ...
                                    {/* <td className="d-flex align-items-center">
                                        {(item.file_path ?
                                            <span className="thumb-img me-2"><img src={"http://localhost:8000/" + item.file_path} className="lesson-img" alt="lesson" /></span>
                                            : ""
                                        )}
                                    </td> */}
                                    <td>
                                        <img src={"https://system-ticketing-backend.onrender.com/uploads/"+item.image} style={{width:'100px',height:'50px'}}/>
                                    </td>
                                    <td>{item.heading}</td>
                                    <td>{item.priority}</td>
                                    <td>{item.category}</td>
                                    ...
                                    {/* <td>{item.description.slice(0, 5)}{item.description.length > 5 ? "..." : ""}</td> */}
                                    ...
                                    {/* <h6
                                     ...
                                     __html: item.description?.description
                                      ...
                                      .slice(0, 5)
                                      .join(" ") +
                                      (item.post_content.english.split(" ").length > 5 ? "......" : "")
                                      : "", // Check if item.post_content?.english is defined
                                     }}
                                    ></h6> */}
                                    ...
                                    <td width="150"><span className="d-flex justify-content-end">
                                        {/* <Link to={"/view-lesson/" + item.id} className="mx-2"><svg width="17" height="12" viewBox="0 0 17 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <Link to={"/edit-ticket/" + item._id} className="mx-2"><svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <Link data-bs-toggle="modal" data-bs-target={`#staticBackdrop_${item._id}`}  className="mx-2"><svg width="14" height="16" viewBox="0 0 14 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <div class="modal fade" id={`staticBackdrop_${item._id}`} data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body py-5">
                                                        ...
                                                        <h5 className="mt-4">Are you sure you want to delete?</h5>
                                                        <div className="d-flex justify-content-center mt-3">
                                                        <button type="button" class="btn btn-default me-3" data-bs-dismiss="modal">No</button>
                                                        <button type="button" class="btn btn-primary" data-bs-dismiss="modal" onClick={() => {deleteTicket(item) } }>Yes</button>
                                                        </div>
                                                        </div>
                                                    </div>
