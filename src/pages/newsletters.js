...
const NewsLetters = () => {
  const [loading, setLoading] = useState(true);
    ...
    const getData = () => {
        setLoading(true);
        DataService.getNewsletter().then((data) => {
            setNewsletter(data?.data?.data);
      ...
      const onChangeSearch =(e)=>{
        if(e.target.value){
            const result = newsletter.filter(value=>{
                return value.email.toLowerCase().includes(e.target.value.toLowerCase());
            })
            setfilteredData(result)
        }else{
            setfilteredData(newsletter)
        }
        
      }
      const deleteNewsletter  = (item) =>{
        setLoading(true);
        DataService.deleteNewsletter(item?.id).then(
            () => {
        ...
        <div className="Right-content">
          ...
          <div className="row">
            <div className="col-md-12">
                <h4 className="f-700 mb-4">Newsletters</h4>
                <div className="table-header d-flex align-items-center">
                    <div className="table-search">
                        ...
                        <input type="search" onChange={onChangeSearch} name="search" placeholder="Search Email" />
                    </div>
                </div>
                                            ...
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body py-5">
                                                        ...
                                                        <h5 className="mt-4">Are you sure you want to delete this?</h5>
                                                        <div className="d-flex justify-content-center mt-3">
                                                        <button type="button" class="btn btn-default me-3" data-bs-dismiss="modal">No</button>
                                                        <button type="button" class="btn btn-primary" data-bs-dismiss="modal" onClick={() => {deleteNewsletter(item) } }>Yes</button>
                                                        </div>
                                                        </div>
                                                    </div>
